...
def TOBCPActivation_ACN_Encode(pVal: TOBCPActivation, codec: ACN, bCheckConstraints: Boolean): Either[ErrorCode, Int] =  // acn:21
{
    ...
    OBCPActivation_obcp_ID_ACN_Encode(codec, OBCPActivation_obcp_ID) match {
        case Left(l) =>
...
@opaque @inlineOnce
def OBCPActivation_obcp_ID_Encode_loop(codec: ACN, OBCPActivation_obcp_ID: Vector[UByte], i1: Int): Either[Int, Int] = {
    require((0 <= i1) && (i1 <= 5))
    ...
    if (i1 == 5) {
        ...
        val res = OBCPActivation_obcp_ID_Encode_loop(codec, OBCPActivation_obcp_ID, i1 + 1)
        res
    }
...
@opaque @inlineOnce
def OBCPActivation_obcp_ID_ACN_Encode(codec: ACN, OBCPActivation_obcp_ID: Vector[UByte]): Either[Int, Int] = {
    require(OBCPActivation_obcp_ID.size == (6))
    require(codec.base.bitStream.validate_offset_bits((7L * 5)))
    OBCPActivation_obcp_ID_Encode_loop(codec, OBCPActivation_obcp_ID, 0)
}.ensuring { (res: Either[Int, Int]) => 
    res match {
        case Left(_) =>
            true
...
def TOBCPActivation_ACN_Decode(codec: ACN): EitherMut[ErrorCode, TOBCPActivation] =
{
    ...
    val OBCPActivation_obcp_ID = OBCPActivation_obcp_ID_ACN_Decode(codec) match {
        case LeftMut(l) =>
            return LeftMut(l)
        case RightMut(bdg) =>
            bdg
...
@opaque @inlineOnce
def OBCPActivation_obcp_ID_Decode_loop(codec: ACN, OBCPActivation_obcp_ID: Vector[UByte], i1: Int): EitherMut[Int, Vector[UByte]] = {
    require((0 <= i1) && (i1 <= 5))
    ...
    (if (i1 == 5) {
        ...
        val res = OBCPActivation_obcp_ID_Decode_loop(codec, OBCPActivation_obcp_ID.append(OBCPActivation_obcp_ID_arr_i1_), i1 + 1)
        ghostExpr {
            (res match {
                case LeftMut(_) =>
                    ()
...
@opaque @inlineOnce
def OBCPActivation_obcp_ID_ACN_Decode(codec: ACN): EitherMut[Int, Vector[UByte]] = {
    require(codec.base.bitStream.validate_offset_bits((7L * 5)))
    OBCPActivation_obcp_ID_Decode_loop(codec, Vector.empty[UByte], 0)
}.ensuring { (res: EitherMut[Int, Vector[UByte]]) => 
    res match {
        case LeftMut(_) =>
            true
...
@ghost @pure
def OBCPActivation_obcp_ID_ACN_Decode_pure(codec: ACN): (ACN, EitherMut[Int, Vector[UByte]]) = {
    require(codec.base.bitStream.validate_offset_bits((7L * 5)))
    val cpy = snapshot(codec)
    val res = OBCPActivation_obcp_ID_ACN_Decode(cpy)
    (cpy, res)
}
