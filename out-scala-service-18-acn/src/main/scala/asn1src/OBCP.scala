...
def TOBCP_ID_ACN_Encode(pVal: TOBCP_ID, codec: ACN, bCheckConstraints: Boolean): Either[ErrorCode, Int] =  // acn:21
{
    ...
    OBCP_TOBCP_ID_pVal_Encode_loop(codec, pVal, 0) match {
        case Left(l) =>
...
@opaque @inlineOnce
def OBCP_TOBCP_ID_pVal_Encode_loop(codec: ACN, pVal: Vector[UByte], i1: Int): Either[Int, Int] = {
    require((0 <= i1) && (i1 <= 5))
    require(pVal.size == (6))
    ...
    if (i1 == 5) {
        ...
        val res = OBCP_TOBCP_ID_pVal_Encode_loop(codec, pVal, i1 + 1)
        res
    }
...
def TOBCP_ID_ACN_Decode(codec: ACN): EitherMut[ErrorCode, TOBCP_ID] =
{
    ...
    @ghost val codec_0_1 = snapshot(codec)
    @ghost val codecBeforeLoop_0 = snapshot(codec)
    val pVal = OBCP_TOBCP_ID_pVal_Decode_loop(codec, Vector.empty[UByte], 0) match {
        case LeftMut(l) =>
            return LeftMut(l)
        case RightMut(bdg) =>
            bdg
...
@opaque @inlineOnce
def OBCP_TOBCP_ID_pVal_Decode_loop(codec: ACN, pVal: Vector[UByte], i1: Int): EitherMut[Int, Vector[UByte]] = {
    require((0 <= i1) && (i1 <= 5))
    ...
    (if (i1 == 5) {
        ...
        val res = OBCP_TOBCP_ID_pVal_Decode_loop(codec, pVal.append(pVal_arr_i1_), i1 + 1)
        ghostExpr {
            (res match {
                case LeftMut(_) =>
                    ()
